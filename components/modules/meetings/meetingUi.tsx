...
import type { ChunkStatus, MeetingStatus } from "@/lib/db/types";
...
const CHUNK_STYLES: Record<ChunkStatus, { label: string; cls: string }> = {
  pending: { label: "pending", cls: "border-edge bg-surface-2/60 text-fg-dim" },
  uploaded: {
    label: "uploaded",
    cls: "border-accent/50 bg-accent/10 text-accent",
  },
...
export function ChunkPill({ status }: { status: ChunkStatus }) {
  const s = CHUNK_STYLES[status] ?? CHUNK_STYLES.pending;
  return (
    <span
      className={`rounded-sm border px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider ${s.cls}`}
    >
