...
export function MeetingsView({ initialMeetings }: Props) {
  ...
  const liveCount = meetings.filter((m) =>
    (["recording", "transcribing", "summarizing"] as MeetingStatus[]).includes(
      m.status,
    ),
  ).length;
        ...
        {showForm && (
          ...
          >
            ...
            <p className="font-mono text-[11px] text-fg-dim leading-relaxed">
              ...
              extract action items into tasks, and update memory. Live recording
              (native system audio) runs from the desktop app.
            </p>
        ...
        {meetings.length === 0 ? (
          <div className="grid place-items-center h-48">
            <p className="font-mono text-sm text-fg-dim text-center">
              // no meetings yet — click + new to summarize a transcript, or start
              a live recording from the desktop app
            </p>
          </div>
        ) : (
